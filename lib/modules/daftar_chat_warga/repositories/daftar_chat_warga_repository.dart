...
class DaftarChatWargaRepository {
  final ApiService _apiService;

  DaftarChatWargaRepository({ApiService? apiService})
  ...
  Future<Map<String, dynamic>> fetchComplaintDetails(String idPengaduan) async {
    final response = await _apiService.dio.get('/pengaduan/$idPengaduan');
    if (response.data['status'] == true) {
      return response.data['data'] ?? {};
    }
    throw response.data['message'] ?? 'Gagal mengambil detail pengaduan';
  }
}
