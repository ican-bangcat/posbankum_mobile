...
--
-- Host: localhost:3306
-- Generation Time: Jun 17, 2026 at 02:37 PM
...
CREATE TABLE `pengaduan` (
  ...
  `masyarakat_id` char(36) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `pengaduan_before_update` BEFORE UPDATE ON `pengaduan` FOR EACH ROW BEGIN
  IF NEW.id_paralegal IS NOT NULL AND NEW.id_paralegal <> '' THEN
    IF (
      SELECT COUNT(*)
      FROM users u
      JOIN posbankum_paralegal pp
        ON pp.id_user = u.id_user
       AND pp.id_posbankum = NEW.id_posbankum
       AND pp.status = 'aktif'
      ...
      SET MESSAGE_TEXT = 'id_paralegal harus user paralegal aktif pada Posbankum yang sama'$$
DELIMITER ;
...
ALTER TABLE `pengaduan`
  ...
  ADD KEY `fk_pengaduan_kabupaten` (`id_kabupaten`),
  ADD KEY `fk_pengaduan_kecamatan` (`id_kecamatan`),
  ADD KEY `fk_pengaduan_kelurahan` (`id_kelurahan`),
  ADD KEY `fk_pengaduan_posbankum` (`id_posbankum`),
  ADD KEY `fk_pengaduan_created_by` (`created_by`),
  ADD KEY `fk_pengaduan_masyarakat` (`masyarakat_id`),
  ADD KEY `fk_pengaduan_paralegal` (`id_paralegal`);
...
--
ALTER TABLE `pengaduan`
  ADD CONSTRAINT `fk_pengaduan_created_by` FOREIGN KEY (`created_by`) REFERENCES `users` (`id_user`) ON DELETE RESTRICT,
  ADD CONSTRAINT `fk_pengaduan_kabupaten` FOREIGN KEY (`id_kabupaten`) REFERENCES `kabupaten` (`id_kabupaten`) ON DELETE SET NULL,
  ADD CONSTRAINT `fk_pengaduan_kecamatan` FOREIGN KEY (`id_kecamatan`) REFERENCES `kecamatan` (`id_kecamatan`) ON DELETE SET NULL,
  ADD CONSTRAINT `fk_pengaduan_kelurahan` FOREIGN KEY (`id_kelurahan`) REFERENCES `kelurahan` (`id_kelurahan`) ON DELETE SET NULL,
  ADD CONSTRAINT `fk_pengaduan_masyarakat` FOREIGN KEY (`masyarakat_id`) REFERENCES `users` (`id_user`) ON DELETE SET NULL,
  ADD CONSTRAINT `fk_pengaduan_paralegal` FOREIGN KEY (`id_paralegal`) REFERENCES `users` (`id_user`) ON DELETE SET NULL,
  ADD CONSTRAINT `fk_pengaduan_posbankum` FOREIGN KEY (`id_posbankum`) REFERENCES `posbankum` (`id_posbankum`) ON DELETE RESTRICT;
