...
| No. Test | Skenario Uji Coba | Langkah-Langkah | Hasil yang Diharapkan | Status |
| :--- | :--- | :--- | :--- | :---: |
| **TC-28** | Masuk ke Ruang Obrolan | 1. Login sebagai Warga.<br>2. Buka detail pengaduan aktif (`diproses`).<br>3. Tap tombol **Hubungi Pendamping / Paralegal**. | • Berhasil membuka halaman Detail Chat.<br>• Header menampilkan nama Paralegal pendamping secara dinamis.<br>• Memuat riwayat obrolan terdahulu jika ada. | `[ ]` |
| **TC-29** | Pengiriman & Penentuan Posisi Pesan (Sender Bubble) | 1. Ketik pesan teks di chat input.<br>2. Kirim pesan. | • Pesan terkirim ke server Laravel.<br>• Pesan pengirim (Warga/Paralegal sendiri) dirender di sisi kanan (bubble gelap).<br>• Pesan penerima dirender di sisi kiri (bubble putih/avatar klien). | `[ ]` |
...
| **TC-32** | Tampilan Jam Terkirim (WhatsApp Style) | 1. Perhatikan posisi jam pada gelembung obrolan pengirim dan penerima. | Jam terkirim berada di dalam bubble chat, merapat rapi di kanan bawah teks obrolan. | `[ ]` |
| **TC-33** | Halaman Informasi Chat Posbankum (POV Warga) | 1. Login sebagai Warga.<br>2. Masuk ke ruang chat, tap ikon menu/info di kanan atas header. | Masuk ke halaman **Info Chat Posbankum** yang menampilkan nama Kelurahan/Posbankum, kontak telepon, alamat, peta lokasi, dan jadwal layanan. | `[ ]` |
| **TC-34** | Halaman Informasi Chat Warga (POV Paralegal) | 1. Login sebagai Paralegal.<br>2. Masuk ke ruang chat, tap ikon menu/info di kanan atas header. | Masuk ke halaman **Info Chat Warga** yang menampilkan NIK klien, nama, email, nomor HP, dan alamat detail klien. | `[ ]` |
| **TC-35** | Pengiriman Lampiran Berkas Media Chat | 1. Tap tombol plus (+) di bagian input.<br>2. Pilih **Pilih Dokumen (PDF)** atau **Gambar**.<br>3. Pilih berkas dan kirim. | • Media terunggah via Multipart ke `/pengaduan/{id}/lampiran` dengan `jenis_lampiran = 'chat'`.<br>• Pesan dikirim berformat khusus: `[FILE]nama_file.ext\|secure_url`. | `[ ]` |
| **TC-36** | Pratinjau Gambar & Pembuka PDF Chat | 1. Di ruang chat, tap bubble media gambar.<br>2. Tap bubble media dokumen PDF. | • Gambar terbuka fullscreen dengan mode cubit-zoom.<br>• PDF langsung terunduh via header secure token (tanpa terhalang 401 redirect) dan tampil di layar `PdfViewerScreen` internal. | `[ ]` |
...
| No. Test | Skenario Uji Coba | Langkah-Langkah | Hasil yang Diharapkan | Status |
| :--- | :--- | :--- | :--- | :---: |
| **TC-37** | Navigasi Halaman Daftar Kegiatan | 1. Login sebagai Paralegal.<br>2. Buka tab/menu **Kegiatan**. | • Masuk ke halaman **Kelola Kegiatan**.<br>• Header melengkung dengan gambar ilustrasi gedung.<br>• Tampilan ter-constraint maksimal `650.0` pada layar lebar/tablet. | `[ ]` |
| **TC-38** | Pencarian & Filter Kegiatan | 1. Ketik kata kunci pada Search Bar.<br>2. Tap tombol filter tanggal (ikon kalender) di kanan search bar.<br>3. Pilih tanggal di kalender modal. | • Daftar kegiatan menyaring secara instan berdasarkan judul/lokasi.<br>• Judul bagian list berubah format (contoh: *DAFTAR KEGIATAN: JULI 2026*).<br>• Tap lagi tombol kalender untuk mereset filter tanggal. | `[ ]` |
| **TC-39** | Tambah Kegiatan Baru (Multipart Upload & Form Validation) | 1. Tap tombol **Mulai buat Laporan kegiatan**.<br>2. Masukkan judul, tgl mulai & selesai, lokasi, jumlah peserta, deskripsi.<br>3. Ketik nama anggota tim terlibat lalu tekan Enter/ikon tambah.<br>4. Tap unggah gambar cover, pilih foto (kamera/galeri).<br>5. Tap **Kirim Laporan**. | • Validasi mewajibkan judul, tanggal, lokasi, dan gambar cover.<br>• Anggota tim ditambahkan dalam bentuk tag/chips reaktif.<br>• Sukses mengunggah ke database server via Multipart, dan status awal kegiatan diset ke **MENUNGGU**. | `[ ]` |
| **TC-40** | Tampilan Detail Laporan Kegiatan | 1. Tap salah satu kegiatan di list Kelola Kegiatan.<br>2. Perhatikan detail informasi yang ditampilkan. | • Layout detail rapi, cover gambar tampil jernih via normalisasi HTTPS.<br>• Menampilkan status (badge), tgl kegiatan, deskripsi, jumlah peserta, hasil kegiatan, dan daftar chips anggota terlibat.<br>• Tombol **Edit Kegiatan** dan **Hapus Kegiatan** tersedia di bagian bawah detail jika status masih "Menunggu". | `[ ]` |
| **TC-41** | Edit Laporan Kegiatan | 1. Di detail kegiatan berstatus "Menunggu", tap **Edit Kegiatan**.<br>2. Ubah beberapa isian form (misal: kurangi anggota, ganti judul/lokasi).<br>3. Klik **Simpan Perubahan**. | Data terupdate secara instan di server Laravel, dan status kegiatan diset ulang kembali ke **MENUNGGU** (untuk verifikasi ulang admin). | `[ ]` |
| **TC-42** | Hapus Laporan Kegiatan | 1. Di detail kegiatan, tap **Hapus Kegiatan**.<br>2. Tap **Ya, Hapus** pada dialog konfirmasi. | • Kegiatan terhapus dari server.<br>• Halaman kembali ke daftar kegiatan utama dan list terupdate secara instan. | `[ ]` |
| **TC-43** | Penampilan Alasan Penolakan (Catatan Admin) | 1. Buka detail kegiatan yang berstatus **DITOLAK**.<br>2. Perhatikan bagian bawah rincian. | Muncul kartu berwarna merah terang bertuliskan **Catatan Admin** yang berisi detail alasan penolakan dari admin website. | `[ ]` |
